...
- **예전(ZooKeeper)**: 메타데이터·컨트롤러 선출·구성 관리를 **별도 ZooKeeper 클러스터**가 담당. 운영 부담 + 메타데이터 확장 한계.
- **KRaft (KIP-500)**: ZooKeeper를 없애고 **Kafka 자체가 Raft 합의**로 메타데이터 관리. **Controller Quorum** 노드가 그 역할.
  - Kafka 3.x부터 production-ready, **Kafka 4.0(2025)부터 ZooKeeper 제거 = KRaft 전용**.
  - 장점: 운영 단순(클러스터 1개), 메타데이터 확장성, 빠른 장애 복구.
...
- 발행 신뢰성은 **Spring Modulith 아웃박스 → externalization** ([도메인 모델링 §7](../system-design/260617-종소세-도메인모델링.md)).
