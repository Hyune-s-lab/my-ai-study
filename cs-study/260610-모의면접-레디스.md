...
| 제약 | — | **멀티 키 연산은 같은 슬롯**이어야(MGET, 트랜잭션, Lua) → **hash tag** `{user:1}:a` 로 슬롯 고정 |
...
- **value에 uuid**: 해제 시 **내 락인지 확인 후 삭제** — Lua로 GET+DEL을 원자 실행. 안 그러면 TTL 만료 후 남의 락을 지우는 사고.

주의점(여기가 본론):

- **TTL보다 작업이 길어지면** 락이 풀려 두 주체가 동시 진입 → Redisson의 **watchdog**(연장) 또는 작업을 TTL 안으로.
...
**감점 포인트**: `SETNX`만 말하고 끝. uuid 검증·Lua 원자 해제·TTL 연장·failover 한계 중 두 개 이상 빠지면 실무에서 사고 낼 답.
...
- 실무에선 **Lua 스크립트**(또는 Redis Function)가 사실상 표준: 스크립트 전체가 원자 실행 + 로직 포함 가능. 단 **스크립트도 싱글 스레드를 점유**하므로 길면 안 됨.
...
- **타임아웃**: command timeout(Lettuce 기본 60s)도 다운스트림 타임아웃이다 — [타임아웃 문서](./260610-모의면접-타임아웃.md)의 outbound 원칙 그대로(짧게, 명시적으로).
...
- **Rate limiting**: 키당 카운터. 고정 윈도우는 `INCR`+`EXPIRE`, 정밀하게는 **sliding window**(ZSET에 타임스탬프, 범위 카운트) 또는 token bucket을 **Lua로 원자 구현**. 분산 인스턴스가 한도를 공유하려면 Redis가 사실상 표준.
- **Idempotency key 저장**: 타임아웃 재시도의 중복 실행 방지(→ [타임아웃 Q5](./260610-모의면접-타임아웃.md)). `SET key result NX EX 86400`.
...
→ 두 명령 사이가 원자적이지 않다 — INCR 후 앱이 죽으면 **TTL 없는 카운터가 영원히 남는다.** Lua로 묶거나 `SET ... EX NX`+`INCR` 패턴, 또는 검증된 구현(Redisson, Bucket4j) 사용.
...
9. 분산락: SET NX PX + uuid + Lua 해제. 효율성용은 OK, 정확성은 fencing token/DB로
10. MULTI/EXEC는 롤백 없음. 원자 로직은 Lua
11. Spring: 직렬화 명시(JSON), Lettuce, @Cacheable TTL 필수, command timeout 명시
12. 게이트웨이: rate limit(Lua 원자) · idempotency key · 응답/semantic 캐시 · 사용량 집계
```
