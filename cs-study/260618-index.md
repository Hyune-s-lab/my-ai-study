...
| **복합(composite)** | 여러 컬럼 묶음. **최좌측 접두**만 활용 | `(user_id, tax_year)` 동시 조건 |
| **UNIQUE** | 유일성 제약 + 인덱스 | `filing.idempotency_key` |
| **부분(partial)** | 조건(`WHERE`) 맞는 행만 인덱싱 → 작고 빠름 | `… WHERE status='ACCEPTED'` |
