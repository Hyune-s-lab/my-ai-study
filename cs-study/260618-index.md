...
| **GIN** | 다중 값 내부 검색 | `jsonb`·배열·전문검색 (`input_snapshot @> …`) |
| **GiST** | 공간·범위·근접 | 위치(PostGIS)·범위 타입 |
| **BRIN** | 거대 + 물리 정렬된 테이블 | 시계열·로그 (작고 가벼움) |

## 인덱스 종류 (개념)

- **단일 / 복합(composite)**: 여러 컬럼 묶음. → **최좌측 접두(leftmost prefix)** 만 활용됨.
- **UNIQUE**: 유일성 제약 + 인덱스. (예: `filing.idempotency_key`)
- **부분(partial)**: 조건부. `WHERE status='ACCEPTED'` → 그 행만 인덱싱(작고 빠름).
- **표현식(expression)**: `lower(email)` 처럼 가공값 인덱싱.
- **커버링(covering, `INCLUDE`)**: 쿼리 컬럼을 인덱스에 다 담아 **Index-Only Scan**(힙 안 감).

## 복합 인덱스 — 컬럼 순서가 핵심
- **등치(`=`) 컬럼 먼저, 범위(`<,>`) 컬럼 나중.** (범위 뒤 컬럼은 인덱스 못 탐)
- `(user_id, tax_year)` 인덱스는 `user_id=?` 나 `user_id=? AND tax_year=?` 엔 타지만, **`tax_year` 단독**엔 못 탐(최좌측 깨짐).
...
## Clustered vs Non-clustered (UUIDv7 논의와 연결)
...
- → 그래서 **UUIDv7**(시간정렬)이 인덱스 지역성에 유리(특히 MySQL). [→ 키 설계](../system-design/260611-종소세-환급-설계.md)
...
## 종소세 설계와 연결
- `filing`: PK `filing_id` · `UNIQUE(idempotency_key)` · 부분 `UNIQUE(user_id,tax_year,type) WHERE status='ACCEPTED'`.
- 조회 패턴별: `(user_id, tax_year)` 복합 인덱스.
- `jsonb`(`input_snapshot`·`result`) 내부 검색 필요하면 **GIN**.
- **파티셔닝**(`tax_year`)과 함께: 파티션별 로컬 인덱스로 인덱스도 작게.
