...
> **5월 신고 시즌의 어느 날 밤 9시, 조회·제출 트래픽이 집중된다.**  
...
%%{init: {"theme": "base", "darkMode": false, "themeVariables": {"background": "#ffffff", "primaryColor": "#EFF6FF", "primaryTextColor": "#16213E", "primaryBorderColor": "#3B5BA5", "secondaryColor": "#F0FDF4", "tertiaryColor": "#FAF5FF", "lineColor": "#3B5BA5", "textColor": "#16213E", "edgeLabelBackground": "#ffffff", "actorBkg": "#EFF6FF", "actorBorder": "#3B5BA5", "actorTextColor": "#16213E", "signalColor": "#3B5BA5", "signalTextColor": "#16213E", "noteBkgColor": "#FFF7ED", "noteBorderColor": "#C98A2B", "noteTextColor": "#16213E", "labelBoxBkgColor": "#EFF6FF", "labelTextColor": "#16213E", "loopTextColor": "#16213E", "actorLineColor": "#64748B", "labelBoxBorderColor": "#3B5BA5"}}}%%
  box rgb(255, 255, 255)
    ...
    participant N as 국세청 외부 연동
  end
  rect rgb(255, 255, 255)
    U->>A: 환급액 조회
    A->>N: 원천 데이터 수집
    Note over A,N: 느린 외부 호출 동안 요청 자원 점유
    N-->>A: 수집 결과
    A->>A: 세액 계산
    A-->>U: 예상 환급액
  end
...
%%{init: {"theme": "base", "darkMode": false, "themeVariables": {"background": "#ffffff", "primaryColor": "#EFF6FF", "primaryTextColor": "#16213E", "primaryBorderColor": "#3B5BA5", "secondaryColor": "#F0FDF4", "tertiaryColor": "#FAF5FF", "lineColor": "#3B5BA5", "textColor": "#16213E", "edgeLabelBackground": "#ffffff", "clusterBkg": "#F8FAFC", "clusterBorder": "#CBD5E1"}}}%%
  subgraph canvas[" "]
    direction TD
    app["사용자 앱"] -->|"잡 생성·상태 조회"| api["API Gateway · Job API"]
    api -->|"잡과 Outbox 저장"| db["PostgreSQL: 잡 상태 · 결과"]
    db --> relay["Outbox Relay"]
    relay -.-> kafka["Kafka: 수집 잡"]
    kafka -.-> worker["수집·계산 Worker"]
    worker --> nts["국세청 외부 연동"]
    nts -->|"원천 데이터"| worker
    worker -->|"상태·결과 저장"| db
  end
  ...
  class relay,worker worker
```


**L0 대비 추가**: API Gateway · Kafka(잡 토픽) · Worker · PostgreSQL(상태 머신).

- 국세청 수집을 **워커 잡**으로 분리 → API는 즉시 `jobId` 반환, 사용자는 폴링으로 결과 수신 ⇒ **1️⃣ 완화**("3분 미리보기"가 비동기로 성립).
- 진행 상태를 **PostgreSQL**(`REQUESTED→COLLECTING→CALCULATING→READY`)에 저장 → 중간 실패해도 이어서 재시도 ⇒ 수집·계산 잡의 복구가 가능하다. 제출 결과 불명 문제인 **4️⃣은 L2에서 다룬다**.
...
- **쓰기 사가(3️⃣4️⃣)**: API Gateway→**제출 서비스**(Spring 서비스, 사가를 코드로 구동·상태는 PostgreSQL): `검증 → 제출(멱등키) → 접수확인 폴링 → 완료기록`, 실패 시 보상. 외부 연동이 멱등키를 지원한다면 같은 키를 재사용하고, 지원하지 않으면 접수 조회·수동 확인 절차를 둔다. 타임아웃 시 **상태 재확인 후 판단**(성공 오판 금지). *(워크플로가 길고 복잡해지면 Temporal 같은 durable workflow 엔진으로 승급)*
...
%%{init: {"theme": "base", "darkMode": false, "themeVariables": {"background": "#ffffff", "primaryColor": "#EFF6FF", "primaryTextColor": "#16213E", "primaryBorderColor": "#3B5BA5", "secondaryColor": "#F0FDF4", "tertiaryColor": "#FAF5FF", "lineColor": "#3B5BA5", "textColor": "#16213E", "edgeLabelBackground": "#ffffff", "clusterBkg": "#F8FAFC", "clusterBorder": "#CBD5E1"}}}%%
  subgraph canvas[" "]
    direction TD
    app["사용자 앱"] --> api["Gateway · Job API"]
    api --> submit["제출 서비스: 멱등 처리 · 상태 조회"]
    api --> db["PostgreSQL: Job · Estimate · Filing · Outbox"]
    db --> relay["Outbox Relay"]
    relay -.-> kafka["Kafka: 잡 · 도메인 이벤트"]
    kafka -.-> worker["Worker: 수집 · 계산"]
    kafka -.-> notify["알림 서비스: FCM · APNs"]
    worker --> guard["외부 호출 보호: timeout · retry · CB · rate limit"]
    submit --> guard
    guard --> nts["국세청 외부 연동"]
    worker --> cache["Redis: 수집 결과 캐시"]
    worker -->|"원천 데이터 보관"| s3["S3"]
    worker -->|"상태·결과"| db
    submit -->|"접수 상태·스냅샷"| db
    worker -.->|"재시도 소진"| dlt["Kafka DLT"]
  end
  style canvas fill:#ffffff,stroke:#ffffff,color:#111827
  linkStyle default stroke:#3B5BA5,stroke-width:1.5px
  classDef app fill:#EFF6FF,stroke:#3B5BA5,stroke-width:1px,color:#16213E
  class app,api,submit,notify,guard,nts app
  classDef db fill:#F0FDF4,stroke:#3F8E55,stroke-width:1px,color:#16213E
  class db,kafka,cache,s3,dlt db
  classDef policy fill:#FAF5FF,stroke:#A855F7,stroke-width:1px,color:#16213E
  classDef worker fill:#FFF7ED,stroke:#C98A2B,stroke-width:1px,color:#16213E
  class relay,worker worker
```
...
| **Kafka** | 잡/이벤트 토픽 + DLT | 비동기·결합도↓ (→ [Kafka 구조](../computer-science/2606-kafka-구조.md)) |

> 규모 근거: 신고/계산은 메시지 수준의 초고 TPS가 아님(시즌 피크도 수백~수천 TPS급) → PostgreSQL + 인덱스 + 읽기 복제본 + Redis로 충분. 정말 더 필요해지면 그때 폴리글랏.

## 도메인 — 바운디드 컨텍스트 / 애그리거트

> 애그리거트·경계·ID 참조의 개념은 [DDD 애그리거트](../computer-science/2606-ddd-aggregate.md) 참고. 아래는 그 개념을 종소세 도메인에 적용한 결과.
...
%%{init: {"theme": "base", "darkMode": false, "themeVariables": {"background": "#ffffff", "primaryColor": "#EFF6FF", "primaryTextColor": "#16213E", "primaryBorderColor": "#3B5BA5", "secondaryColor": "#F0FDF4", "tertiaryColor": "#FAF5FF", "lineColor": "#3B5BA5", "textColor": "#16213E", "edgeLabelBackground": "#ffffff", "clusterBkg": "#F8FAFC", "clusterBorder": "#CBD5E1"}}}%%
  subgraph canvas[" "]
    direction LR
    USERS["USERS"] -->|"1 : 0..N 요청"| ESTIMATE["ESTIMATE"]
    RULESET["RULESET"] -->|"1 : 0..N 계산"| ESTIMATE
    USERS -->|"1 : 0..N 제출"| FILING["FILING"]
    ESTIMATE -->|"1 : 0..N 근거"| FILING
  end
  style canvas fill:#ffffff,stroke:#ffffff,color:#111827
  linkStyle default stroke:#3B5BA5,stroke-width:1.5px
  classDef app fill:#EFF6FF,stroke:#3B5BA5,stroke-width:1px,color:#16213E
  class USERS,ESTIMATE,RULESET,FILING app
  classDef db fill:#F0FDF4,stroke:#3F8E55,stroke-width:1px,color:#16213E
  classDef policy fill:#FAF5FF,stroke:#A855F7,stroke-width:1px,color:#16213E
  classDef worker fill:#FFF7ED,stroke:#C98A2B,stroke-width:1px,color:#16213E
```
...
동의·보상·증적은 별도 엔티티로 둔다. `DOMAIN_EVENT.aggregate_id`는 애그리거트 유형과 함께 해석하는 다형 참조다.
...
%%{init: {"theme": "base", "darkMode": false, "themeVariables": {"background": "#ffffff", "primaryColor": "#EFF6FF", "primaryTextColor": "#16213E", "primaryBorderColor": "#3B5BA5", "secondaryColor": "#F0FDF4", "tertiaryColor": "#FAF5FF", "lineColor": "#3B5BA5", "textColor": "#16213E", "edgeLabelBackground": "#ffffff", "clusterBkg": "#F8FAFC", "clusterBorder": "#CBD5E1"}}}%%
  subgraph canvas[" "]
    direction LR
    USERS["USERS"] -->|"1 : 0..N 동의 기록"| CONSENT["CONSENT"]
    USERS -->|"1 : 0..N 청구"| CLAIM["CLAIM"]
    FILING["FILING"] -->|"1 : 0..N 근거"| CLAIM
  end
  style canvas fill:#ffffff,stroke:#ffffff,color:#111827
  linkStyle default stroke:#3B5BA5,stroke-width:1.5px
  classDef app fill:#EFF6FF,stroke:#3B5BA5,stroke-width:1px,color:#16213E
  class USERS,CONSENT,CLAIM,FILING app
  classDef db fill:#F0FDF4,stroke:#3F8E55,stroke-width:1px,color:#16213E
  classDef policy fill:#FAF5FF,stroke:#A855F7,stroke-width:1px,color:#16213E
  classDef worker fill:#FFF7ED,stroke:#C98A2B,stroke-width:1px,color:#16213E
```
...
> 인덱스 종류·설계 원칙은 [인덱스](../computer-science/2606-index.md), 파티셔닝 vs 샤딩은 [샤딩 vs 파티셔닝](../computer-science/2606-sharding-partitioning.md) 참고.
...
**기술 선택**
- **1차 = 도메인 이벤트 로그**: `FilingSubmitted`·`ClaimApproved` 같은 **비즈니스 사실**을 불변 기록. **Spring Modulith 이벤트 발행 레지스트리(아웃박스) → Kafka + `domain_event` 테이블**. (→ [Kafka 구조](../computer-science/2606-kafka-구조.md))
