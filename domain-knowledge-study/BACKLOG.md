...
> 현재 학습 맥락: [어텐션](./260603-어텐션-기초.md) → [KV 캐시](./260601-kv-cache-기초.md) → [모델 종류](./260603-llm-모델-종류.md) → [서빙 최적화](./260603-llm-서빙-최적화-라우팅-disaggregation.md)
...
- [ ] **추론(inference)이 되는 전체 과정**
  ...
  - "모델이 다음 토큰을 고른다"의 실제 계산 경로 → 관련: [어텐션](./260603-어텐션-기초.md), [KV 캐시](./260601-kv-cache-기초.md), [dictionary 샘플링](./dictionary.md)
...
- [ ] **token per second(TPS)를 늘리는 방법 총정리**
  ...
  - 측정 기준/벤치 방법(어떤 워크로드·동시성에서) → 관련: [서빙 최적화 §2](./260603-llm-서빙-최적화-라우팅-disaggregation.md)
...
- [ ] **worker 여러 개일 때 cache hit율 검증 방법**
  ...
  - 어디서 깨지는지(prefix 1토큰 차이, replica 분산) 관측 → 관련: [서빙 최적화 §3](./260603-llm-서빙-최적화-라우팅-disaggregation.md)

- [ ] **vLLM vs SGLang**
  - 핵심 차이: APC(블록 해싱) vs RadixAttention(토큰 radix tree), 기능/생태계, 라우터
  - 언제 무엇을 고르나 (공유 prefix 많은 멀티턴·에이전트 = SGLang? 범용 = vLLM?)
  - PD disaggregation·양자화·spec decoding 지원 현황 비교 → 관련: [서빙 최적화 §2.1](./260603-llm-서빙-최적화-라우팅-disaggregation.md)
