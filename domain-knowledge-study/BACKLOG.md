...
- **서빙 관측(observability) — 어느 축이 병목인지 측정**
  ...
  - 엔진 메트릭: vLLM `/metrics`(`time_to_first_token`·`time_per_output_token`·`gpu_cache_usage_perc`·`num_requests_waiting`·`num_preemptions_total`)
  - 병목 진단: VRAM·큐 → 용량 / `DRAM_ACTIVE`↑ → 대역폭(decode) / `SM·TENSOR_ACTIVE`↑ → 연산(prefill). TTFT vs TPOT로도 구분 (※ nvidia-smi GPU-Util은 부정확 — 믿지 말 것)
